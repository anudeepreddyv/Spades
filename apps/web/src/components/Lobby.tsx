      ...
      >
        ...
        {[
          ...
          },
          {
            ...
            body: 'Winning more tricks than you bid earns you a bag. Bags are tracked and displayed next to your score (e.g. "120 +2B"). Every 3 bags costs you a 30-point penalty and those bags reset. So 4 bags = −30 pts and 1 bag remaining.',
          },
      ...
      <div style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: 460, padding: '0 16px' }}>
        ...
        <div style={{ background: C.card, backdropFilter: 'blur(20px)', borderRadius: 20, border: `1px solid ${C.border}`, padding: '28px 28px 24px', boxShadow: '0 32px 80px rgba(0,0,0,0.6)' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 22 }}>
            {(['create', 'join', 'cpu'] as const).map(t => (
              <button key={t} onClick={() => setTab(t as any)} style={{
                ...
                letterSpacing: '0.1em', textTransform: 'uppercase' as const, transition: 'all 0.2s',
                ...
                {t === 'create' ? 'New Game' : t === 'join' ? 'Join Game' : '🤖 vs CPU'}
              </button>
            ))}
          </div>
          ...
          {tab === 'create' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <div style={{ background: 'rgba(245,200,66,0.05)', border: '1px solid rgba(245,200,66,0.12)', borderRadius: 8, padding: '10px 14px', display: 'flex', justifyContent: 'space-around' }}>
                {[['13', 'Rounds'], ['×10', 'Per bid'], ['Bags', 'Tracked separately']].map(([val, lbl]) => (
          ...
          {tab === 'cpu' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              {/* CPU info card */}
              <div style={{ background: 'rgba(147,197,253,0.06)', border: '1px solid rgba(147,197,253,0.18)', borderRadius: 12, padding: '18px 16px', textAlign: 'center' }}>
                <div style={{ fontSize: 44, marginBottom: 10 }}>🤖</div>
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, fontWeight: 700, color: '#93c5fd', marginBottom: 8 }}>
                  Play vs Computer
                </div>
                <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: 'rgba(180,210,255,0.6)', lineHeight: 1.6 }}>
                  Go head-to-head against CPU in a solo 2-player game. Full 13-round match — all the same rules, no waiting for others.
                </div>
              </div>

              {/* CPU difficulty display */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '12px 16px' }}>
                <span style={{ fontSize: 20 }}>🎯</span>
                <div>
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, color: 'rgba(200,225,255,0.85)' }}>Smart AI Opponent</div>
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: 'rgba(160,190,220,0.5)', marginTop: 2 }}>Bids based on hand strength · Plays strategically · Tries to win every trick it can</div>
                </div>
              </div>

              <button onClick={handleVsComputer} disabled={!name.trim() || !connected}
                onMouseEnter={() => setHoveredBtn('cpu')} onMouseLeave={() => setHoveredBtn(null)}
                style={{
                  width: '100%', padding: '14px 0', borderRadius: 12, border: 'none',
                  cursor: name.trim() && connected ? 'pointer' : 'not-allowed',
                  background: hoveredBtn === 'cpu' && name.trim() ? 'linear-gradient(135deg,#60a5fa,#818cf8)' : 'linear-gradient(135deg,#3b82f6,#6366f1)',
                  color: '#fff', fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 700,
                  letterSpacing: '0.15em', textTransform: 'uppercase' as const, transition: 'all 0.2s',
                  opacity: name.trim() && connected ? 1 : 0.4,
                  boxShadow: name.trim() && connected ? '0 0 24px rgba(99,102,241,0.45)' : 'none',
                }}>
                🤖 Start vs CPU
              </button>
            </div>
